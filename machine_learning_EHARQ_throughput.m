function final_throughput=machine_learning_EHARQ_throughput(noise)
...
enb.NDLRB =22;                % No of Downlink RBs in total BW
...
while feedback==0 && count<4
    ...
    feedback=lr.predict({{retrans,rvIndex-1,SNR,25}}).int64;
    retrans=1;
    fprintf("\n"+feedback+"\n");
    %if feedback==0
    %    continue;
...
throughput(j)=(total_received/total_sent)*100
j=j+1;
